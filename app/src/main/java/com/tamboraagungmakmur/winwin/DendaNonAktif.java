...
public class DendaNonAktif extends Fragment {
                ...
                try {
                    ...
                    if (klienResponse.getData().length != 0) {
                        ...
                        for (int i = 0; i < klienResponse.getData().length; i++) {
                            getEmail(klienResponse.getData()[i].getId(), klienResponse.getData()[i].getNomor_pengajuan());
                        }
...
//                DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
    ...
    private void getPengajuan2(final String cari) {
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.GET, AppConf.URL_DENDA_FINDNONAKTIF + query + "?_session=" + sessionId, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
