...
public class RefJenisBayarEditActivity extends FragmentActivity {

    private static final Object TAG = "REF_JENIS_BAYAR_EDIT";
    @Bind(R.id.save)
    Button save;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        context = RefJenisBayarEditActivity.this;
...
//
                        ...
                        ArrayAdapter<CharSequence> adapter = new ArrayAdapter(RefJenisBayarEditActivity.this, android.R.layout.simple_spinner_item, tipe);
                        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
...
//        VolleyHttp.getInstance(context).addToRequestQueue(stringRequest);
    ...
    private void getDetail() {
        Log.d("klien_all", id);
        SessionManager sessionManager = new SessionManager(context);
        StringRequest stringRequest = new StringRequest(Request.Method.GET, AppConf.URL_REF_JENIS_BAYAR_DETAIL + "/" + id + "?_session=" + sessionManager.getSessionId(), new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                ...
                try {
                    JSONObject jsonObject1 = new JSONObject(response);
                    RefJenisBayar klienResponse = new Gson().fromJson(jsonObject1.getString("data"), RefJenisBayar.class);

                    if (klienResponse != null) {
                        txNamaKode.setText(klienResponse.getJenisbayar_code());
                        txNamaLabel.setText(klienResponse.getJenisbayar_label());
                        txKeterangan.setText(klienResponse.getJenisbayar_penjelasan());


                        for (int i = 0; i < dataArrayList.size(); i++) {
                            if (dataArrayList.get(i).getTrxtype_label().equalsIgnoreCase(klienResponse.getStat_trx())) {
                                spinnerMan.setSelection(i);
                            }
                        }

                        String rlc[] = getResources().getStringArray(R.array.status_bank);
                        int pos1 = Arrays.asList(rlc).indexOf(klienResponse.getStat_rlc());
...
//        VolleyHttp.getInstance(context).addToRequestQueue(stringRequest);
    ...
    private void edit() {
        ...
        SessionManager sessionManager = new SessionManager(context);
        StringRequest stringRequest = new StringRequest(Request.Method.PATCH, AppConf.URL_REF_JENIS_BAYAR_UPDATE+ "/" + id +"?_session=" + sessionManager.getSessionId(), new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
