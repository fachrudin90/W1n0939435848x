...
public class HomeJatuhTempo extends Fragment {
    ...
    private final static String TAG = "HOME_ANALISA";
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        try {
            view = inflater.inflate(R.layout.fragment_home_analisa, container, false);
        ...
        total.setText("Total Task Jatuh Tempo: 0");
...
//        GetData();
        ...
        LocalBroadcastManager.getInstance(view.getContext()).registerReceiver(mMessageReceiver1,
                new IntentFilter("task_jatuhtempo1"));
...
//        VolleyHttp.getInstance(mActivity).getRequestQueue().cancelAll(TAG);
    ...
    private void getTask() {
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.GET, AppConf.URL_TASK_JATUHTEMPO + "/" + LIMIT + "/" + offset + "?_session=" + sessionManager.getSessionId(), new Response.Listener<String>() {
            @Override
...
//                    JSONArray jsonArray = new JSONArray(klienResponse.getData().toString());
                    if (klienResponse.getData().length != 0) {
                        ...
                        if (total != null) {
                            total.setText("Total Task Jatuh Tempo: " + klienResponse.getData()[0].getCount());
                        }
...
//                    mActivity.finish();
    ...
    private void removeTask(final String idTask) {
        Log.d("task_all", "ok");

        SessionManager sessionManager = new SessionManager(mActivity);
        StringRequest stringRequest = new StringRequest(Request.Method.DELETE, AppConf.URL_TASK_REMOVES + "/" + idTask + "?_session=" + sessionManager.getSessionId(), new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {

                Log.d("task_remove", response);
                swipeRefreshLayout.setRefreshing(false);
                try {
                    JSONObject jsonObject1 = new JSONObject(response);
...
//                    mActivity.finish();
    ...
    private void getTask1(final String tgl1, final String tgl2) {
        ...
        SessionManager sessionManager = new SessionManager(mActivity);
        StringRequest stringRequest = new StringRequest(Request.Method.GET, AppConf.URL_TASK_JATUHTEMPO + "/" + LIMIT + "/" + offset + "/" + tgl1 + "/" + tgl2 + "?_session=" + sessionManager.getSessionId(), new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
...
//        VolleyHttp.getInstance(mActivity).addToRequestQueue(stringRequest);
    ...
    private void getTask2(final String cari) {
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.GET, AppConf.URL_TASK_JATUHTEMPO_FIND + query + "?_session=" + sessionManager.getSessionId(), new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
