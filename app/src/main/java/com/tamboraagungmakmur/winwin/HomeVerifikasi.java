 ...
 */
public class HomeVerifikasi extends Fragment {
    ...
    private final static String TAG = "HOME_VERIFIKASI";
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        try {
            view = inflater.inflate(R.layout.fragment_home_verifikasi, container, false);
        ...
        total.setText("Total Task Verifikasi: 0");
...
//        GetData();
        ...
        LocalBroadcastManager.getInstance(view.getContext()).registerReceiver(mMessageReceiver1,
                new IntentFilter("task_verifikasi1"));
...
//        VolleyHttp.getInstance(mActivity).getRequestQueue().cancelAll(TAG);
    ...
    private void getTask() {
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.GET, AppConf.URL_TASK_VERIFIKASI + "/" + LIMIT + "/" + offset + "?_session=" + sessionManager.getSessionId(), new Response.Listener<String>() {
            @Override
...
//                    JSONArray jsonArray = new JSONArray(klienResponse.getData().toString());
                    if (klienResponse.getData().length != 0) {
                        ...
                        if (total != null) {
                            total.setText("Total Task Verifikasi: " + klienResponse.getData()[0].getCount());
                        }
...
//        VolleyHttp.getInstance(mActivity).addToRequestQueue(stringRequest);
    ...
    private void getTask1(final String tgl1, final String tgl2) {
        ...
        SessionManager sessionManager = new SessionManager(mActivity);
        StringRequest stringRequest = new StringRequest(Request.Method.GET, AppConf.URL_TASK_VERIFIKASI + "/" + LIMIT + "/" + offset + "/" + tgl1 + "/" + tgl2 + "?_session=" + sessionManager.getSessionId(), new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
...
//        VolleyHttp.getInstance(mActivity).addToRequestQueue(stringRequest);
    ...
    private void getTask2(final String cari) {
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.GET, AppConf.URL_TASK_VERIFIKASI_FIND + query + "?_session=" + sessionManager.getSessionId(), new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
